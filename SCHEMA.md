...
| `elf_dynsym_unread` | `.dynsym` would not read, named strings `.dynstr` does not hold, declared fewer entries than `.dynstr` holds names for, or a section named `.dynsym` exists whose declared `sh_type` is not `SHT_DYNSYM` and so cannot be trusted as one, so the imported-versus-defined split is missing or partial. |
